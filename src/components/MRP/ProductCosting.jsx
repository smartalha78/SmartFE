...
const API_CONFIG = {
    ...
    GET_BOM_FOR_SUMMARY: `${API_BASE1}/get-bom-for-summary`,
    ...
    SAVE_COST_SHEET: `${API_BASE1}/save-cost-sheet`
};
...
/* ---------------------------
 * BOM Detail Row Component
---------------------------- */
const BOMDetailRow = ({ index, data }) => {
    const isLowStock = parseFloat(data.availableQty || 0) < parseFloat(data.requiredQty || 0);
    
    return (
        <div className={`pc-bom-row ${isLowStock ? 'pc-low-stock' : ''}`}>
            <div className="pc-bom-cell pc-bom-index">{index + 1}</div>
            <div className="pc-bom-cell pc-bom-process">{data.processName || '-'}</div>
            <div className="pc-bom-cell pc-bom-material">
                <div className="pc-bom-material-code">{data.rawMaterialCode}</div>
                <div className="pc-bom-material-name">{data.rawMaterialName}</div>
            </div>
            <div className="pc-bom-cell pc-bom-uom">{data.uomName || data.uom}</div>
            <div className="pc-bom-cell pc-bom-qty">{formatNumber(data.requiredQty)}</div>
            <div className="pc-bom-cell pc-bom-price">{formatNumber(data.costPrice)}</div>
            <div className="pc-bom-cell pc-bom-value">{formatNumber(data.value)}</div>
            <div className={`pc-bom-cell pc-bom-available ${isLowStock ? 'pc-warning' : ''}`}>
                {formatNumber(data.availableQty)}
...
const ProcessOverheadRow = ({ index, process, rate, defaultRate, onRateChange, canEdit }) => {
    ...
    const processName = process.ProcessName || process.cname || `Process ${processId}`;
...
const ProductCosting = () => {
    ...
    const [rateType, setRateType] = useState('profile'); // 'profile', 'lastSale', 'average'
    ...
    const loadProcessesWithRates = async (productCode = null) => {
        try {
            ...
            if (data.success) {
                ...
                // Initialize process rates with default workflow rates
                const initialRates = {};
    ...
    // This function is for the Required Summary tab (shows BOM without process/FOH costs)
    const handleCalculateRequiredSummary = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                setMessage('✅ Required summary calculated successfully');
    ...
    // This function is for the complete BOM cost with process and FOH
    ...
    const handleProcessRateChange = (process, rate) => {
        ...
        // Recalculate total process cost based on production quantity
    ...
    const printReport = () => {
        ...
        printWindow.document.write(`
            ...
            <body>
                ...
                <table>
                    <thead>
                        <tr><th>#</th><th>Material Code</th><th>Material Name</th><th>UOM</th><th>Required Qty</th><th>Cost Price</th><th>Value</th><th>Available Qty</th></tr>
                    </thead>
                    <tbody>
                        ${bomDetails.map((item, idx) => `
                            <tr>
                                ...
                                <td>${item.uomName || item.uom}</td>
                                <td style="text-align:right">${formatNumber(item.requiredQty)}</td>
                                <td style="text-align:right">${formatNumber(item.costPrice)}</td>
                                <td style="text-align:right">${formatNumber(item.value)}</td>
                                <td style="text-align:right">${formatNumber(item.availableQty)}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
                                    ...
                                    <tbody>
                                        {reportData.map((item, idx) => (
                                            <tr key={idx}>
                                                ...
                                                <td>{item.rawMaterialName}</td>
                                                <td>{item.uomName || item.uom}</td>
                                                <td className="pc-text-right">{formatNumber(item.requiredQty)}</td>
                                                <td className="pc-text-right">{formatNumber(item.costPrice)}</td>
                                                <td className="pc-text-right">{formatNumber(item.value)}</td>
                                                <td className="pc-text-right">{formatNumber(item.availableQty)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>

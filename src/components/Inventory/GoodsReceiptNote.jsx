...
const GRNScreen = () => {
  ...
  const loadCustomers = async () => {
    try {
      setLoading(true);
      const data = await fetchJson(`${API_BASE_URL}/get-suppliers`, {
        method: "GET",
      });

      if (data.success && data.data) {
        setCustomers(data.data);
        console.log(`Loaded ${data.data.length} suppliers`);
      } else {
        console.warn("No suppliers found, using fallback data");
        setCustomers([
          { CustomerCode: "0000000011", CustomerName: "Fauji Foods Limited", country: "Pakistan", city: "LAHORE", id: "1" },
          { CustomerCode: "0000001000", CustomerName: "DG Cement Company", country: "Pakistan", city: "KARACHI", id: "2" },
          { CustomerCode: "0000001001", CustomerName: "Best Suppliers", country: "Pakistan", city: "LAHORE", id: "3" },
          { CustomerCode: "0000001002", CustomerName: "ABC Traders", country: "Pakistan", city: "ISLAMABAD", id: "4" },
          { CustomerCode: "0000001003", CustomerName: "XYZ Enterprises", country: "Pakistan", city: "LAHORE", id: "5" },
        ]);
      }
    } catch (err) {
  ...
  const loadGrnList = async () => {
    try {
      setLoading(true);
      const data = await fetchJson(`${API_BASE_URL}/get-grn-table-data`, {
        method: "POST",
        body: JSON.stringify({
  ...
  const handleEdit = async (grn) => {
    try {
      ...
      const data = await fetchJson(`${API_BASE_URL}/get-grn-with-details`, {
        method: "POST",
        body: JSON.stringify({
          vno: grnNo,
          vockey: vockey,
      ...
      if (data.success && data.data) {
        ...
        setGrnHeader({
          grnNo: headData.vno || grnNo,
          grnDate: headData.vdate ? headData.vdate.split(' ')[0] : new Date().toISOString().split('T')[0],
          supplierCode: headData.suppliercode || "",
          supplierName: headData.suppliername || "",
          supplierCountry: headData.country || "",
          supplierCity: headData.city || "",
  ...
  const handleSave = async () => {
    try {
      ...
      if (editMode === "edit" && grnHeader.grnNo) {
        headData.vno = grnHeader.grnNo;
        headData.vockey = grnHeader.vockey || `${grnHeader.offcode || "0101"}${grnHeader.grnNo}`;
      }
      ...
      console.log(`GRN ${editMode === "edit" ? "Update" : "Insert"} Payload:`, JSON.stringify(payload, null, 2));

      const apiUrl = editMode === "edit" && grnHeader.grnNo
        ? `${API_BASE_URL}/update-grn-table-data`
        : `${API_BASE_URL}/insert-grn-head-det`;
              ...
              <div className="form-group">
                ...
                >
                  ...
                  {customers.map((customer, idx) => (
                    <option key={customer.id || customer.CustomerCode || idx} value={customer.CustomerCode}>
                ...
                {currentItems.map((grn, idx) => (
                  <tr key={grn.vno || grn.id || idx}>
                    ...
                    <td className="action-buttons">
                      <button onClick={() => handleEdit(grn)} className="btn-edit">
                        <FaEdit /> Edit
                      </button>
                      <button onClick={() => generatePDF(grn.vno, grn.vockey)} className="btn-pdf">
                        <FaFilePdf /> PDF
                      </button>
                    </td>
                  </tr>

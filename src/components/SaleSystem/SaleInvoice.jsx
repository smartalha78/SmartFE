...
const SaleInvoiceScreen = () => {
  ...
  const loadGrnList = async () => {
    try {
      setLoading(true);
      const data = await fetchJson(`${API_BASE_URL}/get-saleinvoice-table-data`, {
        method: "POST",
        body: JSON.stringify({
          tableName: "invsalehead",
          offcode: "0101",
        }),
      });
  ...
  const handleEdit = async (grn) => {
    try {
      ...
      const data = await fetchJson(`${API_BASE_URL}/get-saleinvoice-with-details`, {
        method: "POST",
        body: JSON.stringify({
          vno: grnNo,
          vockey: vockey,
      ...
      if (data.success && data.data) {
        ...
        setsaleHeader({
          ...
          CustomerCode: headData.CustomerCode || "",
  ...
  const handleSave = async () => {
    try {
      ...
      const headData = {
        ...
        vtype: "SIN",
        Ptype: 110,
        CustomerCode: saleHeader.CustomerCode,
      ...
      const apiUrl = editMode === "edit" && saleHeader.invoiceNo
        ? `${API_BASE_URL}/update-saleinvoice-table-data`
        : `${API_BASE_URL}/insert-saleinvoice-head-det`;
          ...
          <div className="form-section">
            ...
            <div className="form-row">
              <button
                ...
                onClick={() => loadPurchaseOrders(saleHeader.CustomerCode)}
                disabled={!saleHeader.CustomerCode}
              >
        ...
        <>
          ...
          <div className="grn-list-container">
            <table className="grn-list-table">
              ...
              <tbody>
                {currentItems.map((SaleInvoice, idx) => (
                  <tr key={SaleInvoice.vno || SaleInvoice.id || idx}>
                    <td>{SaleInvoice.vno}</td>
                    <td>{SaleInvoice.vdate}</td>
                    <td>{SaleInvoice.customername}</td>
                    <td>{(parseFloat(SaleInvoice.NetAmount) || 0).toFixed(2)}</td>
                    <td>
                      <span className={`status ${SaleInvoice.status === 1 ? 'posted' : 'unposted'}`}>
                        {SaleInvoice.status === 1 ? 'Posted' : 'Unposted'}
                      </span>
                    </td>
                    <td className="action-buttons">
                      <button onClick={() => handleEdit(SaleInvoice)} className="btn-edit">
                        <FaEdit /> Edit
                      </button>
                      <button onClick={() => generatePDF(SaleInvoice.vno, SaleInvoice.vockey)} className="btn-pdf">
                        <FaFilePdf /> PDF
                      </button>
                    </td>
                  </tr>
...
export default SaleInvoiceScreen;